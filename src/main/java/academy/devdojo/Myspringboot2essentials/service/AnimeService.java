...
public class AnimeService {
	...
	/*
	 * @Transactional(rollbackFor = Exception.class) public Anime
	 * save(AnimePostRequestBodyDTO animePostRequestBodyDTO) throws Exception {
	 * Anime anime =
	 * Anime.builder().name(animePostRequestBodyDTO.getName()).build();
	 * animeRepository.save(anime); if(true) { throw new
	 * Exception("Exception Test!"); } return anime; }
	 */
	...
	public void replace(AnimePutRequestBodyDTO animePutRequestBodyDTO) {
		...
		// Anime newAnime =
		// Anime.builder().name(animePutRequestBodyDTO.getName()).id(animeSalvo.getId()).build();
	...
	public Anime findByNameFirst(String name) {

		return animeRepository.findByName(name).get(0);
	}
